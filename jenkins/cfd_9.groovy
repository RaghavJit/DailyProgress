pipeline {
    ...
    stages {
            ...
            steps {
                dir('cfd_repo') {
                    checkout scmGit(
                        ...
                        extensions: [], 
                        userRemoteConfigs: [[
                            url: 'https://github.com/fossee/CFD_Drupal10/'
                            ]]
                        )
                    }
             }
            ...
            steps {
                dir('cfd_db') {
                    checkout scmGit(
                        ...
                        userRemoteConfigs: [[
                            credentialsId: 'cfd',
                            url: 'https://github.com/RaghavJit/cfd_db'
                        ]]
                    )
                }
            }
        ...
        stage ('Create new MySQL DB') {
            steps {
                script {
                    sh "git config --global --add safe.directory ${WORKSPACE}/cfd_repo"
                    def commitMsg = sh(
                        script: "git -C ${WORKSPACE}/cfd_repo log -1 --pretty=%B",
                        returnStdout: true
                    ).trim()
                        ...
                        sh '''
                            ...
                            if [ -n "$db_list" ]; then
                                while IFS= read -r db; do
                                    suffix="${db#cfd_db_jenkins_}"
                                    user="cfd_user_jenkins_$suffix"
...
EOF
                            ...
                            if [ -f "cfd_db/cfd_10.sql" ]; then
                                rm -rf /tmp/cfd_clean.sql
                                sed -e '/^CREATE DATABASE/d' -e '/^USE[[:space:]]/d' cfd_db/cfd_10.sql > /tmp/cfd_clean.sql
                                mysql -u "$newuser" -p"$SITE_DB_PASSWD" "$newdb" < /tmp/cfd_clean.sql

                            else 
                                echo 'No file cfd_10.sql found'
                            fi
                        '''
                    }
                }
